...
public final class NumberUtils {
     ...
     * formatString: #,###0.0
     */
    public static final String DECIMAL_FORMAT_N1 = ConstantUtils.getValue("#,###0.0");

    /**
     * formatString: #,###0.00
     */
    public static final String DECIMAL_FORMAT_N2 = ConstantUtils.getValue("#,###0.00");

    /**
     * formatString: #,###0.00##
     */
    public static final String DECIMAL_FORMAT_N4 = ConstantUtils.getValue("#,###0.0000");
     ...
     * @return String
     */
    public static String format(final BigDecimal numberValue, final String format) {
     ...
     * @return 限制后的值
     */
    public static Integer limitRange(Integer num, final Integer min, final Integer max) {
        if (num == null) {
            num = min;
        }
        if (num < min) {
     ...
     * @return sum
     */
    public static double multi(final double... values) {
    ...
    public static long round(final double d) {
        final BigDecimal bd = new BigDecimal(String.valueOf(d));
    ...
    public static double round(final double d, final int scale) {
        final BigDecimal bd = new BigDecimal(String.valueOf(d)).setScale(scale, BigDecimal.ROUND_HALF_UP);
     ...
     *
     * @param round    boolean的中文名称：是否四舍五入并精确到分
     * @param numArray []的中文名称：或任意个数的参数,例如 sum(num1,num2,num3),sum(num1,num2)
     * @return BigDecimal
     */
     ...
     *
     * @param round       boolean的中文名称：是否四舍五入并精确到分
     * @param master      BigDecimal
     * @param bigDecimals BigDecimal的中文名称：数组或任意个数的参数,例 subtract(num1,num2)=num1-num2,subtract
     ...
     *
     * @param num1         BigDecimal
     * @param scale        int的中文名称：小数位数
     * @param roundingMode int的中文名称：默认采用4舍5入，见@link BigDecimal
     * @param num2         BigDecimal
     * @return BigDecimal
     */
    public static BigDecimal divide(final BigDecimal num1, final int scale, final int roundingMode, final BigDecimal... num2) {
     ...
     *
     * @param num1           BigDecimal
     * @param throwException boolean的中文名称：除数为0时，是否抛出异常
     * @param scale          int的中文名称：小数位数
     * @param roundingMode   int的中文名称：默认采用4舍5入，见@link BigDecimal
     * @param num2           BigDecimal
     * @return BigDecimal
     */
    public static BigDecimal divide(final BigDecimal num1, final boolean throwException, final int scale, final int roundingMode, final BigDecimal... num2) {
     ...
     *
     * @param data ：要做非空处理的BigDecimal数据
     * @return BigDecimal
     */
     ...
     * @return int
     */
    public static int divideCeil(final int num, final int divNum) {
        if (num % divNum == 0) {
            return num / divNum;
        } else {
            return num / divNum + 1;
    ...
    /**
     * 判断两个Integer是否相等（同时考虑其为0的情况） 1.如果都为null则认为相等 2.如果其中一个为null则认为不相等 3.如果都不为null直接使用原本的equals方法
     *
     * @param num1 Long
     * @param num2 Long
     * @return boolean
     */
    public static boolean compareConsiderNull(final Long num1, final Long num2) {
