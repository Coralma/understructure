...
public final class EnumUtils {
     ...
     * @return 枚举常量
     */
    public static <T extends Enum<T>> T getValueOf(final Class<T> enumClass, final String value) {
        for (final T enumValue : enumClass.getEnumConstants()) {
            if (enumValue.toString().equals(value)) {
                return enumValue;
            }
        }
        throw new IllegalArgumentException(value + " is not valid of enumtype " + enumClass);
    }
     ...
     * @param <T>          枚举的类型参数
     */
    public static <T extends Enum<T>> void changeNameTo(final T enumInstance, final String value) {
        try {
            final Field fieldName = enumInstance.getClass().getSuperclass().getDeclaredField("name");
     ...
     */
    public static boolean enumEquals(final Enum enum1, final Enum enum2) {
