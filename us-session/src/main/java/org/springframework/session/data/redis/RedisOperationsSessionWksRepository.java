...
/**
 * <p>
 * A {@link SessionRepository} that is implemented
 * using Spring Data's
 * {@link RedisOperations}. In a web
 ...
 * {@link RedisSession#getMaxInactiveIntervalInSeconds()}
 * . For example:
 * </p>
 *
 ...
 * @author Rob Winch
 */
public class RedisOperationsSessionWksRepository implements SessionRepository<RedisOperationsSessionWksRepository.RedisSession> {
    /**
    ...
    public RedisOperationsSessionWksRepository(RedisConnectionFactory redisConnectionFactory) {
     ...
     */
    public RedisOperationsSessionWksRepository(RedisOperations<String, ExpiringSession> sessionRedisOperations) {
     ...
     * @return
     */
    public String createAndSaveSession(Map<String, Object> map) {
        RedisSession redisSession = createSession();
        if (map != null) {
            for (String key : map.keySet())
                redisSession.setAttribute(key, map.get(key));
        }
        save(redisSession);
        return redisSession.getId();
    }
     ...
     * {@link RedisSession#saveDelta()} is invoked
