...
import org.springframework.session.web.http.SessionRepositoryWksFilter;
import org.springframework.util.ClassUtils;

/**
 * Exposes the {@link SessionRepositoryWksFilter} as a bean named
...
@Configuration
@EnableScheduling
public class RedisHttpSessionWksConfiguration implements ImportAware, BeanClassLoaderAware {
    ...
    @Bean
    public RedisOperationsSessionWksRepository sessionRepository(RedisTemplate<String, ExpiringSession> sessionRedisTemplate) {
        RedisOperationsSessionWksRepository sessionRepository = new RedisOperationsSessionWksRepository(sessionRedisTemplate);
        sessionRepository.setDefaultMaxInactiveInterval(maxInactiveIntervalInSeconds);
        return sessionRepository;
    }

    @Bean
    public <S extends ExpiringSession> SessionRepositoryWksFilter<? extends ExpiringSession> springSessionRepositoryFilter(SessionRepository<S> sessionRepository, ServletContext servletContext) {
        SessionRepositoryWksFilter<S> sessionRepositoryFilter = new SessionRepositoryWksFilter<S>(sessionRepository);
