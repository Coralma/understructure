...
/**
 * Switches the {@link HttpSession} implementation to be backed by a {@link Session}.
 * The {@link SessionRepositoryWksFilter} wraps the {@link HttpServletRequest} and overrides the methods
 * to get an {@link HttpSession} to be backed by a {@link Session} returned
 * by the {@link SessionRepository}.
 * The {@link SessionRepositoryWksFilter} uses a {@link HttpSessionStrategy} (default {@link CookieHttpSessionStrategy}  to
 * bridge logic between an {@link HttpSession} and the {@link Session}
 * abstraction. Specifically:
 * <ul>
 * <li>The session id is looked up using {@link HttpSessionStrategy#getRequestedSessionId(HttpServletRequest)}.
 * The default is to look in a cookie named SESSION.</li>
 * <li>The session id of newly created {@link ExpiringSession} is sent to the client using
 * <li>The client is notified that the session id is no longer valid with {@link HttpSessionStrategy#onInvalidateSession(HttpServletRequest, HttpServletResponse)}</li>
 * </ul>
 *
 * @author Rob Winch
 * @since 1.0
 */
@Order(SessionRepositoryWksFilter.DEFAULT_ORDER)
public class SessionRepositoryWksFilter<S extends ExpiringSession> extends OncePerRequestFilter {
    ...
    private final String SESSION_PARAMETER = "sessionDispatch";
     ...
     */
    public SessionRepositoryWksFilter(SessionRepository<S> sessionRepository) {
    ...
    /**
     * A {@link HttpServletRequest} that retrieves the {@link HttpSession} using a
    ...
    private final class SessionRepositoryRequestWrapper extends HttpServletRequestWrapper {
        ...
        public String getRequestedSessionId() {
            String sessionDispatch = getRequest().getParameter(SESSION_PARAMETER);
            if (sessionDispatch != null && !"".equals(sessionDispatch))
